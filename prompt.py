"""
LLM(System/User)에게 전달될 프롬프트 템플릿 및 데이터 컨텍스트 정의
...
3. Explicit Logic: 애매한 비즈니스 로직(예: 어떤 단가를 쓸 것인가)을 프롬프트 레벨에서 확정합니다.
"""
...
2. **분석 요청**: 데이터 분석이 필요한 경우 `PythonAnalysisRequest` 도구를 사용하세요.
   - 단순 조회, 계산, 통계 추출 등 Python 코드로 해결 가능한 모든 작업이 포함됩니다.
